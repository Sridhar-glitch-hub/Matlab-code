...
Gs(1:3,1:3) = gamma;      % source couples to dots 1‑3
Gd(4:5,4:5) = gamma;      % drain couples to dots 4‑5
...
if isempty(p) || p.NumWorkers ~= 8
   ...
   % Retarded green function for 5QD(3,2)
        A = zeros(5);

        A(1,1)=w(j)-epsilon+delta+1i*gamma/2;
        A(1,2)=-t_k*exp(1i*phi/5)+1i*gamma/2;
        A(1,3)=1i*gamma/2;
        A(1,5)=-t_k*exp(-1i*phi/5);
        A(2,1)=-t_k*exp(-1i*phi/5)+1i*gamma/2;
        A(2,2)=w(j)-epsilon+1i*gamma/2;
        A(2,3)=-t_k*exp(1i*phi/5)+1i*gamma/2;
        A(3,1)=1i*gamma/2;
        A(3,2)=-t_k*exp(-1i*phi/5)+1i*gamma/2;
        A(3,3)=w(j)-epsilon-delta+1i*gamma/2;
        A(3,4)=-t_k*exp(1i*phi/5);
        A(4,3)=-t_k*exp(-1i*phi/5);
        A(4,4)=w(j)-epsilon+1i*gamma/2;
        A(4,5)=-t_k*exp(1i*phi/5)+1i*gamma/2;
        A(5,1)=-t_k*exp(1i*phi/5);
        A(5,4)=-t_k*exp(-1i*phi/5)+1i*gamma/2;
